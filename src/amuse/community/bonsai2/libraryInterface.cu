...
vector<ullong>    starids(0);       // list of identifiers
...
int getCurrentStateToHost()
{
  ...
  //   bonsai->desort_bodies(bonsai->localTree);
     ...
     memcpy(&starids[0], &bonsai->localTree.bodies_ids[0], sizeof(starids[0])*n_bodies);
...
int initialize_code()
{
  ...
  cudaContext.create(logFile, false);
  cudaContext.createQueue(devID);
  cudaContext.setLogPreamble("AMUSE-run ");
 ...
 //Creat the octree class and set the properties
  ...
  bonsai = new octree(
          0,
          &cudaContext,
          argv,devID, theta, eps, snapshotFile, snapshotIter, 
          0.0,      //quickdump
          0.1,      //ratio
          true,     //sync
          false,    //use mpiio
          ...
          1,        //rebuild rate
          false,    //direct
          0);       //shrd-pid for Bonsai file output
  bonsai->load_kernels();
...
int commit_particles()
{
  ...
  for(int i=0; i < n_bodies; i++)
  {
    ...
    bonsai->localTree.bodies_time[i] = make_float2(bodies_time[i].x, bodies_time[i].y);
  }
...
{
    float t = bonsai->getDt();
    fprintf(stderr,"Returned time from bonsai: %f \n", t);
    double td = t;
  *_timestep = td;
  return 0;
}

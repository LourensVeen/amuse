...
class Parameters(object):
    ...
    def __init__(self, definitions, instance):
        ...
        object.__setattr__(
            self, "_mapping_from_name_to_definition", OrderedDictionary()
        )
    ...
    def get_parameter(self, name):
        if not name in self._mapping_from_name_to_definition:
            raise exceptions.AmuseException(
                "{0!r} not defined as parameter".format(name)
            )
    ...
    def send_cached_parameters_to_code(self):
        cached_parameters = [
            x for x in self.iter_parameters() if x.definition.is_cached()
        ]
    ...
    def check_defaults(self):
        for x in self.iter_parameters():
            ...
            if not value == default_value:
                print(
                    "!default value is not equal to value in code: {0}".format(
                        x.definition.name
                    )
                )
...
class ParametersMemento(object):
    ...
    def __init__(self, mapping_from_name_to_value=None):
        ...
        object.__setattr__(
            self, "_mapping_from_name_to_value", mapping_from_name_to_value
        )
    ...
    def __getattr__(self, name):
        if not name in self._mapping_from_name_to_value:
            raise exceptions.CoreException(f"tried to get unknown parameter '{name}'")

        return self._mapping_from_name_to_value[name]

    def __setattr__(self, name, value):
        if not name in self._mapping_from_name_to_value:
            raise exceptions.CoreException(f"tried to set unknown parameter '{name}'")
    ...
    class _ParametersMetaclass(type):
        def _get_doc(self):
            ...
            for parameter_definition in definitions:
                ...
                output += (
                    " (default value:"
                    + str(parameter_definition.default_value)
                    + ")\n\n"
                )
            return output
    ...
    class ParametersWithDocs(Parameters, metaclass=_ParametersMetaclass):
        def _get_doc(self):
            ...
            for parameter_definition in definitions:
                ...
                output += (
                    " (default value:"
                    + str(self.get_default_value_for(parameter_definition.name))
                    + ")\n\n"
                )
            return output
...
def new_parameters_with_units_converted_instance_with_docs(original, converter):
    class _ParametersMetaclass(type):
        def _convert_from_target_to_source_if_needed(value):
            if isinstance(value, bool) or isinstance(
                value, quantities.NonNumericQuantity
            ):
                return value
            else:
    ...
    class ParametersWithDocs(
        ParametersWithUnitsConverted, metaclass=_ParametersMetaclass
    ):
        def _get_doc(self):
            output = "Parameters: \n"
            for parameter_definition in original._definitions:
...
class ParametersWithUnitsConverted(object):
    ...
    def __getattr__(self, name):
        return self.convert_from_target_to_source_if_needed(
    ...
    def get_default_value_for(self, name):
        return self.convert_from_target_to_source_if_needed(
            self._original.get_default_value_for(name)
        )
...
class AliasParameterDefinition(AbstractParameterDefinition):
    ...
    def get_default_value(self, parameter_set):
        return parameter_set.get_parameter(
            self.aliased_name
        ).definition.get_default_value(parameter_set)
...
class ParameterException(AttributeError):
    template = (
        "Could not {0} value for parameter '{1}' of a '{2}' object, got errorcode <{3}>"
    )
...
class ModuleMethodParameterDefinition(ParameterDefinition):
    def __init__(
        ...
        ParameterDefinition.__init__(
            self, name, description, default_value, must_set_before_get
        )
...
class ModuleBooleanParameterDefinition(ModuleMethodParameterDefinition):
    ...
    def get_value(self, parameter, object):
        return (
            True
            if ModuleMethodParameterDefinition.get_value(self, parameter, object)
            else False
        )

    def set_value(self, parameter, object, bool):
        return ModuleMethodParameterDefinition.set_value(
            self, parameter, object, 1 if bool else 0
        )


class ModuleCachingParameterDefinition(ParameterDefinition):
    def __init__(
        self, functionname, parameter_name, name, description, default_value=None
    ):
        ParameterDefinition.__init__(
            self, name, description, default_value, must_set_before_get=True
        )
        self.stored_value = None
        self.parameter_name = parameter_name
